...
export const createRange = async (
  ...
  const nodes = [document.getElementById(lowerValueID!), document.getElementById(upperValueID!)];
  ...
  toysAmountSlider.noUiSlider!.on('update', function (values: Array<string>, handle: number) {
    ...
    let currentData: Isettings = JSON.parse(localStorage.getItem('settings')!);
